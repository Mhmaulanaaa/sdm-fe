...
const menus: Menu[] = [
  ...
  },
  {
    ...
    icon: "heroicons:clock",
    children: [
      { name: "Hari Aktif", icon: "heroicons:clock", to: "/hari-aktif" },
...
function isActiveMenu(item: Menu): boolean {
  if (item.to) return currentPath.value.startsWith(item.to);
  if (item.children) {
        ...
        <template v-for="(item, index) in menus" :key="item.name">
          ...
          <NuxtLink
            ...
            :class="[
              ...
              isActiveMenu(item)
                ? 'bg-linear-to-r from-green-500 to-emerald-600 text-white shadow-lg scale-[1.02]'
          ...
          <div v-else>
            ...
            >
              ...
              <span :class="[isCollapsed ? 'lg:hidden' : 'flex-1 text-left']">
                {{ item.name }}
              </span>
            ...
            <Transition name="fade">
              ...
              >
                <template v-for="(child, cIndex) in item.children" :key="child.name">
                  <!-- CHILD TANPA SUB -->
                  <NuxtLink
                    v-if="!child.children"
                    :to="child.to"
                    class="flex items-center gap-3 px-3 py-4 rounded-xl text-sm"
                    :class="[
                  ...
                  <!-- CHILD PUNYA SUB (LEVEL 2) -->
                  <div v-else>
                    <button
                      @click="toggleDropdown(child.name + cIndex)"
                      class="w-full flex items-center gap-3 px-3 py-4 rounded-xl text-sm text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-800"
                    >
                      <UIcon :name="child.icon" class="w-4 h-4" />
                      ...
                      <UIcon
                        ...
                        :class="{
                          'rotate-180': isDropdownOpen(child.name + cIndex),
                        }"
                      />
                    </button>
                    ...
                    <Transition name="fade">
                      <div
                        v-show="isDropdownOpen(child.name + cIndex)"
                      ...
                      >
                        <NuxtLink
                          ...
                          :key="sub.name"
                          :to="sub.to"
                          class="block px-3 py-4 rounded-lg text-sm"
                          :class="[
